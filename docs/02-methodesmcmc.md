...
Ce chapitre est accompagné d’une application interactive (`shinyApp`) pour illustrer concrètement le fonctionnement des algorithmes MCMC. Elle permet de visualiser les trajectoires des chaînes, le mélange, l’autocorrélation et l’impact des réglages sur la qualité de l’échantillonnage. L'application est fournie dans le script `02-mcmc.R` et peut être lancée depuis `R` avec :
