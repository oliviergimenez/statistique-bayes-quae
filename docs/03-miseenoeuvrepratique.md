...
#> Intercept    -0.71      0.28    -1.28    -0.18 1.00     1900     2262
#> 
...
La moyenne a posteriori vaut -0.71 bien loin de la proportion de ragondins qui ont survécu à l'hiver ($19/57 \approx 0.33$). Comme toujours dans `R` et l'implémentation des modèles linéaires généralisés (voir Chapitre \@ref(glms)), les estimations des paramètres sont données sur l'échelle de la fonction de lien. Ici l'intercept estimé est exprimé sur l’échelle logit. Pour le convertir en probabilité de survie (entre 0 et 1), on extrait d'abord les valeurs générées dans la distribution a posteriori de l'intercept $\beta$ avec la fonction `brms::as_draws_matrix()` : 
...
#>        0%       25%       50%       75%      100% 
#> 0.1523343 0.2897609 0.3303362 0.3729360 0.5521512
```
...
#> 1 Intercept 0.333  0.330 0.0611 0.0618 0.235 0.437  1.00    1900.    2262.
```
...
#>   variable   mean median    sd   mad    q5   q95  rhat ess_bulk ess_tail
#>   <chr>     <dbl>  <dbl> <dbl> <dbl> <dbl> <dbl> <dbl>    <dbl>    <dbl>
#> 1 Intercept 0.920  0.903 0.159 0.153 0.691  1.21  1.00    1900.    2262.
```
...
#> Regression Coefficients:
#>           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
#> Intercept    -0.68      0.28    -1.25    -0.16 1.00     1681     2185
#> 
#> Draws were sampled using sampling(NUTS). For each parameter, Bulk_ESS
#> and Tail_ESS are effective sample size measures, and Rhat is the potential
#> scale reduction factor on split chains (at convergence, Rhat = 1).
