...
Ce chapitre est accompagné d’une application interactive (`shinyApp`) dédiée à la régression linéaire. Elle permet de simuler des données, d’ajuster un modèle avec `brms`, et d’explorer l’effet des paramètres, des priors et de la taille d’échantillon sur les résultats. L'application est fournie dans le script `05-lms.R` et peut être lancée depuis `R` avec :
