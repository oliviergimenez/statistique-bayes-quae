...
```{r ragondinos, fig.cap = "Cliché de ragondins (Myocastor coypus) pris sur le bassin versant du Lez dans les environs de Montpellier. Crédits : Yann Raulet.", echo = FALSE}
...
```{r survie-vraisemblance-mle, echo = FALSE, fig.cap = "Fonction de vraisemblance pour la probabilité de survie hivernale du ragondin, calculée à partir de $y=19$ survivants sur $n=57$ individus suivis par GPS. Le maximum de vraisemblance est indiqué par une ligne pointillée rouge."}
y <- 19
n <- 57
grille <- seq(0, 1, 0.01)
...
```{r beta-exemples, echo = FALSE, fig.cap = "Exemples de lois bêta pour différentes valeurs des paramètres $a$ et $b$. Dans chaque panneau, les zones ombrées illustrent la probabilité d'observer une valeur dans un intervalle donné."}
...
```{r posterior-survie, echo = FALSE, fig.cap = "Distribution a priori uniforme (rouge) et distribution a posteriori (noire) de la probabilité de survie hivernale du ragondin. La ligne bleue pointillée correspond à l'estimateur du maximum de vraisemblance."}
df_post <- data.frame(
  theta = x,
  prior = dbeta(x, 1, 1),
