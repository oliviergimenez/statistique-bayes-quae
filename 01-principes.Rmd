...
bin1 <- rbinom(100,50,0.5) %>%
  ...
  labs(title = expression("Binomiale avec n = 50 et " * theta * " = 0.5", x = NULL, y = NULL)) +
  theme_clean
...
bin2 <- rbinom(100,50,0.2) %>%
  ...
  ggplot(aes(x = value, y = n)) +
  geom_col(fill = "#E69F00", color = "black") +
  labs(title = expression("Binomiale avec n = 50 et " * theta * " = 0.2", x = NULL, y = NULL)) +
  theme_clean
...
bin3 <- rbinom(100,50,0.8) %>%
  ...
  ggplot(aes(x = value, y = n)) +
  geom_col(fill = "#E69F00", color = "black") +
  labs(title = expression("Binomiale avec n = 50 et " * theta * " = 0.8", x = NULL, y = NULL)) +
  theme_clean
...
```{r posterior-survie, echo = FALSE, fig.cap = "Distribution a priori uniforme (noir) et distribution a posteriori (rouge) de la probabilité de survie hivernale du ragondin. La ligne bleue pointillée correspond à l'estimateur du maximum de vraisemblance.", dpi = 600}
df_post <- data.frame(
  theta = x,
  prior = dbeta(x, 1, 1),
...
df_post %>%
  ggplot(aes(x = theta)) +
  geom_line(aes(y = prior), size = 1.2) +
