...
```{r hist-surviebrms, fig.cap = "Histogramme de la distribution a posteriori de la probabilité de survie (\\(\\theta\\))."}
draws_fit %>%
  ggplot(aes(x = theta)) +
...
```{r trace-surviebrms, fig.cap = "Densité a posteriori et trace plot de la probabilité de survie sur l'échelle logit (\\(\\beta\\))."}
plot(bayes.brms)
```
...
```{r hist-life, fig.cap = "Histogramme de la distribution a posteriori de l'espérance de vie."}
lambda %>%
  as_tibble() %>%
  ggplot() +
