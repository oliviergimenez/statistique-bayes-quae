...
```{r priors-comparaison, echo=FALSE, fig.cap = "Effet combiné du prior et de la taille d’échantillon sur la distribution a posteriori avec une vraisemblance binomiale. En colonnes : trois lois bêta a priori Beta(1,1), Beta(5,5) et Beta(20,1). En lignes : petit (n = 6, y = 2) et grand (n = 57, y = 19) échantillon (facteur 10). Le trait rouge représente le prior, le trait noir la distribution a posteriori.", out.width = "100%", dpi = 600}
...
```{r informative-vs-vague, eval=FALSE, fig.align='center', fig.cap="Comparaison des distributions postérieures : prior vague (bleu) vs. prior informatif (rouge) pour trois années de données.", include=FALSE, out.width='90%'}
...
```{r prior-normal-viz, echo=FALSE, fig.cap = "Distribution normale avec une moyenne de 0.05 et un écart-type de 0.156. L’intervalle ombré correspond à l’intervalle de crédibilité à 80 %, entre –0.15 et 0.25.", fig.align='center', out.width='100%', dpi = 600}
library(ggplot2)
...
```{r prior-combined-ggplot, fig.cap="Comparaison de deux priors obtenus sur la probabilité \\( \\theta = \\text{logit}^{-1}(\\beta) \\) après transformation par la fonction logit réciproque de \\( \\beta \\sim N(0, 10^2) \\) et \\( \\beta \\sim N(0, 1.5^2) \\). L'axe des abscisses représente les différentes valeurs possibles de la probabilité \\( \\theta \\) obtenues après transformation par la fonction logit inverse. L'axe des ordonnées indique la fréquence des tirages simulés pour chaque valeur.", out.width='100%', fig.align='center', message=FALSE, warning=FALSE, echo = FALSE, dpi = 600}
library(ggplot2)
library(dplyr)
