...
```{r prior-normal-viz, echo=FALSE, fig.cap = "Distribution normale avec une moyenne de 0.05 et un écart-type de 0.156. L’intervalle ombré correspond à l’intervalle de crédibilité à 80 %, entre –0.15 et 0.25.", fig.align='center', out.width='100%'}
library(ggplot2)
...
```{r prior-combined-ggplot, fig.cap="Comparaison de deux priors obtenus sur la probabilité \\( \\theta = \\text{logit}^{-1}(\\beta) \\) après transformation par la fonction logit réciproque de \\( \\beta \\sim N(0, 10^2) \\) et \\( \\beta \\sim N(0, 1.5^2) \\).", out.width='100%', fig.align='center', message=FALSE, warning=FALSE, echo = FALSE}
library(ggplot2)
library(dplyr)
