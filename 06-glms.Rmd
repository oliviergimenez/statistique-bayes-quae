...
```{r gelman-rule, fig.cap="Illustration de la règle du 4 de Gelman : la pente maximale de la courbe logistique est approchée en divisant le coefficient estimé par 4. Ici, on approxime l'effet du poids du ragondin sur la probabilité de survie autour du point d'inflexion.", echo=FALSE, dpi = 600}
# Extraire les coefficients postérieurs moyens
post <- fixef(fit_logit)
intercept <- post["Intercept", "Estimate"]
slope <- post["poids", "Estimate"]

# Règle du 4 de Gelman
approx_slope <- slope / 4
p0 <- plogis(intercept)  # probabilité au point d'inflexion (x = 0 si centré)

# Séquence de poids centrée pour le graphique
poids_seq <- seq(0, 15, length.out = 100)
proba <- plogis(intercept + slope * poids_seq)

# Créer le graphique
df_pred <- data.frame(poids = poids_seq, proba = proba)

ggplot(df_pred, aes(x = poids, y = proba)) +
  geom_line(size = 1.2) +
  geom_abline(intercept = p0, slope = approx_slope, color = "red", linetype = "dashed") +
  labs(x = "Poids (kg)", y = "Probabilité de survie") +
  theme_minimal()
```
...
```{r logit-vs-gaussian, fig.cap="Comparaison entre une régression linéaire et une régression logistique ajustées sur des données binaires. La régression linéaire (en bleu) peut produire des prédictions négatives ou plus grandes que 1 (embêtant pour une probabilité de survie), tandis que la régression logistique (en rouge) garantit une estimation de probabilité valide.", echo=FALSE, dpi = 600}
# Créer une séquence de poids pour prédiction
newdata <- data.frame(poids = seq(0, 15, length.out = 100))
...
#df_pred <- newdata %>%
#  mutate(gaussian = pred_lm[, "Estimate"],
#         gaussian_lower = pred_lm[, "Q2.5"],
#         gaussian_upper = pred_lm[, "Q97.5"],
#         bernoulli = pred_logit[, "Estimate"],
#         bernoulli_lower = pred_logit[, "Q2.5"],
#         bernoulli_upper = pred_logit[, "Q97.5"]) %>%
#  pivot_longer(cols = c(gaussian, bernoulli),
#               names_to = "model", values_to = "estimate") %>%
#  mutate(lower = c(pred_lm[, "Q2.5"], pred_logit[, "Q2.5"]),
#         upper = c(pred_lm[, "Q97.5"], pred_logit[, "Q97.5"]))
df_lm <- data.frame(poids = newdata$poids,
                    estimate = pred_lm[,'Estimate'],
                    model = "Régression linéaire (normale)")

df_logit <- data.frame(poids = newdata$poids,
...
ggplot() +
  geom_line(data = df_plot, aes(x = poids, y = estimate, color = model),
            size = 1.2) +
...
```{r protocole, fig.cap="Schéma des données sur les ragondins selon un protocole d’échantillonnage avec 10 points dans 10 transects. En haut on a le nombre de ragondins, et en bas la température.", echo = FALSE, dpi = 600}
library(ggplot2)
library(dplyr)
library(ggforce)
library(patchwork)  # pour mettre les figures côte à côte
...
# Ajouter température et abondance simulée
transect_inside <- transect_inside |>
  mutate(
...
# Panel 1 : Anémones
p1 <- ggplot() +
  geom_polygon(data = study_area, aes(x = X, y = Y), fill = NA, color = "black") +
  geom_path(data = transect_inside, aes(x = X, y = Y, group = Transect), color = "grey50") +
  geom_point(data = transect_inside, aes(x = X, y = Y, color = Ragondins), size = 2) +
  scale_color_viridis_c(option = "plasma") +
  labs(title = "Nombre de ragondins", x = "X (m)", y = "Y (m)", color = "Ragondins") +
  coord_fixed() +
  theme_minimal()
...
p2 <- ggplot() +
  geom_polygon(data = study_area, aes(x = X, y = Y), fill = NA, color = "black") +
  geom_path(data = transect_inside, aes(x = X, y = Y, group = Transect), color = "grey50") +
  geom_point(data = transect_inside, aes(x = X, y = Y, color = Temperature), size = 2) +
  scale_color_viridis_c(option = "viridis") +
  labs(title = "Température", x = "X (m)", y = "Y (m)", color = "Température (°C)") +
  coord_fixed() +
  theme_minimal()
...
```{r plotsimple, echo=FALSE, fig.cap="Relation entre le nombre de ragondins et la température par transect, avec plusieurs points de comptage (10 pour tous, sauf les transects 4, 5 et 8 pour lesquels on a 3, 2 et 3 points) par transect.", fig.align='center', dpi = 600}
sim_simple %>%
...
```{r ppcheck-complete, fig.cap="Vérification de l'adéquation du modèle avec complete pooling ou regroupement complet. Les distributions simulées (en bleu) sont comparées aux données observées (en noir). Le mauvais recouvrement indique une mauvaise adéquation du modèle aux données.", echo=FALSE, dpi = 600}
...
```{r ppcheck-nopool, fig.cap="Vérification de l'adéquation du modèle avec no pooling ou pas de regroupement. Les distributions simulées (en bleu) sont comparées aux données observées (en noir). Le mauvais recouvrement indique une mauvaise adéquation du modèle aux données.", echo=FALSE, dpi = 600}
...
```{r pooling-ragondins, fig.cap="Comparaison entre les modèles complete pooling (noir) et no pooling (rouge) pour prédire le nombre de ragondins en fonction de la température, par transect. Le modèle no pooling ajuste une courbe indépendante pour chaque transect, tandis que le complete pooling suppose une relation commune.", echo=FALSE, dpi = 600}
# Données pour prédire : séquence de températures par transect
# grid <- expand.grid(
#   Temp = seq(min(sim_simple$Temp), max(sim_simple$Temp), length.out = 100),
#   Transect = unique(sim_simple$Transect)
# )
...
#   geom_line(data = grid, aes(y = y_complete), color = "black", size = 0.8) +
#   geom_line(data = grid, aes(y = y_nopool), color = "red", size = 0.8) +
#   facet_wrap(~fTransect, ncol = 5, scales = "free") +
#   labs(title = "",
#        x = "Température (°C)", y = "Nombre de ragondins") +
#   theme_minimal()
...
```{r model-diagnostics, fig.cap="Vérification de la convergence des chaînes MCMC.", dpi = 600}
plot(fit_partial)
```

On peut alors mettre à jour la Figure \@ref(fig:pooling-ragondins) avec la Figure \@ref(fig:partial-ragondins) :
```{r partial-ragondins, fig.cap="Comparaison entre les modèles complete pooling (noir), no pooling (rouge) et partial pooling (bleu) pour prédire le nombre de ragondins en fonction de la température, par transect. Le modèle no pooling ajuste une courbe indépendante pour chaque transect, le complete pooling suppose une relation commune, tandis que le partial pooling fournit un compromis grâce à l'effet aléatoire transect.", echo=FALSE, dpi = 600}
# données pour prédire : séquence de températures par transect
# grid <- expand.grid(
#   Temp = seq(min(sim_simple$Temp), max(sim_simple$Temp), length.out = 100),
#   Transect = unique(sim_simple$Transect)
...
```{r ppcheck-partial, fig.cap="Vérification de l'adéquation du modèle avec partial pooling ou regroupement partiel. Les distributions simulées (en bleu) sont comparées aux données observées (en noir). Le mauvais recouvrement indique une mauvaise adéquation du modèle aux données.", echo=FALSE, dpi = 600}
...
```{r hist-b0-reel, fig.cap = "Distribution a posteriori de l'intercept moyen (échelle réelle). La ligne rouge indique la vraie valeur (0).", message = FALSE, warning = FALSE, dpi = 600}
tibble(b0 = bzero) %>%
  ggplot(aes(x = b0)) +
...
```{r hist-b1-reel, fig.cap = "Distribution a posteriori de l'effet de la température (échelle réelle). La ligne rouge indique la vraie valeur (0.2).", message = FALSE, warning = FALSE, dpi = 600}
tibble(b1 = bun) %>%
  ggplot(aes(x = b1)) +
  geom_histogram(color = "white", fill = "skyblue", bins = 30) +
