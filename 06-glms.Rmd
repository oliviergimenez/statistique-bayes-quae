...
```{r protocole, fig.cap="Schéma des données sur les ragondins selon un protocole d’échantillonnage avec 10 points dans 10 transects. L'aire d'étude est en noir. En haut on a le nombre de ragondins, et en bas la température.", echo = FALSE}
library(ggplot2)
library(dplyr)
library(patchwork)
...
```{r plotsimple, echo=FALSE, fig.cap="Relation entre le nombre de ragondins et la température par transect, avec plusieurs points de comptage (10 pour tous, sauf les transects 4, 5 et 8 pour lesquels on a 3, 2 et 3 points) par transect.", fig.align='center'}
sim_simple %>%
...
```{r ppcheck-complete, fig.cap="Vérification de l'adéquation du modèle avec complete pooling ou regroupement complet. Les distributions simulées (en bleu) sont comparées aux données observées (en noir). Le mauvais recouvrement indique une mauvaise adéquation du modèle aux données.", echo=FALSE}
...
```{r ppcheck-nopool, fig.cap="Vérification de l'adéquation du modèle avec no pooling ou pas de regroupement. Les distributions simulées (en bleu) sont comparées aux données observées (en noir). Le mauvais recouvrement indique une mauvaise adéquation du modèle aux données.", echo=FALSE}
...
```{r pooling-ragondins, fig.cap="Comparaison entre les modèles complete pooling (noir) et no pooling (rouge) pour prédire le nombre de ragondins en fonction de la température, par transect. Le modèle no pooling ajuste une courbe indépendante pour chaque transect, tandis que le complete pooling suppose une relation commune.", echo=FALSE}
...
```{r model-diagnostics, fig.cap="Vérification de la convergence des chaînes MCMC pour le modèle avec partial pooling."}
plot(fit_partial)
```

On peut alors mettre à jour la Figure \@ref(fig:pooling-ragondins) avec la Figure \@ref(fig:partial-ragondins) :
```{r partial-ragondins, fig.cap="Comparaison entre les modèles complete pooling (noir), no pooling (rouge) et partial pooling (bleu) pour prédire le nombre de ragondins en fonction de la température, par transect. Le modèle no pooling ajuste une courbe indépendante pour chaque transect, le complete pooling suppose une relation commune, tandis que le partial pooling fournit un compromis grâce à l'effet aléatoire transect.", echo=FALSE}
# données pour prédire : séquence de températures par transect
# grid <- expand.grid(
#   Temp = seq(min(sim_simple$Temp), max(sim_simple$Temp), length.out = 100),
#   Transect = unique(sim_simple$Transect)
...
```{r ppcheck-partial, fig.cap="Vérification de l'adéquation du modèle avec partial pooling ou regroupement partiel. Les distributions simulées (en bleu) sont comparées aux données observées (en noir). Le mauvais recouvrement indique une mauvaise adéquation du modèle aux données.", echo=FALSE}
...
```{r hist-b0-reel, fig.cap = "Distribution a posteriori de l'intercept moyen (échelle réelle). La ligne rouge indique la vraie valeur (0).", message = FALSE, warning = FALSE}
tibble(b0 = bzero) %>%
  ggplot(aes(x = b0)) +
...
```{r hist-b1-reel, fig.cap = "Distribution a posteriori de l'effet de la température (échelle réelle). La ligne rouge indique la vraie valeur (0.2).", message = FALSE, warning = FALSE}
tibble(b1 = bun) %>%
  ggplot(aes(x = b1)) +
  geom_histogram(color = "white", fill = "skyblue", bins = 30) +
