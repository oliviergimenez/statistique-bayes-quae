...
```{r fig-posterior-regression, echo = TRUE, fig.cap = "Histogrammes des distributions a posteriori (colonne de gauche) et traces (colonne de droite) des paramètres de la régression linéaire.", dpi = 600}
plot(lm.brms)
```
...
```{r ppcheck-brms, echo = TRUE, fig.cap = "Posterior predictive checks réalisés avec brms. La courbe noire correspond aux données observées, les courbes bleues aux données simulées selon le modèle.", dpi = 600}
pp_check(lm.brms)
```
