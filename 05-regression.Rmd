...
```{r donnees-simulees, echo = FALSE, fig.cap = "Données simulées (n = 100) selon le modèle \\(y_i = \\beta_0 + \\beta_1 x_i + \\varepsilon_i\\), avec \\(\\beta_0 = 0.1\\), \\(\\beta_1 = 1\\) et \\(\\sigma = 1\\). La droite rouge correspond à la droite de régression.", dpi = 600}
...
```{r fig-prior-regression-vague, echo = TRUE, fig.cap = "Simulation de droites de régression issues des distributions a priori. Chaque ligne correspond à un tirage des paramètres : intercept et pente ~ N(0, 100).", dpi = 600}
...
```{r fig-prior-regression, echo = FALSE, fig.cap = "Simulation de droites de régression issues des distributions a priori. Chaque ligne correspond à un tirage des paramètres : intercept et pente ~ N(0, 1.5).", dpi = 600}
...
```{r fig-prior-sigma, fig.cap = "Comparaison entre deux lois a priori pour l’écart-type \\(\\sigma\\) : une loi uniforme \\(\\text{U}(0,5)\\), qui donne la même densité entre 0 et 5, et une loi exponentielle \\(\\text{Exp}(1)\\), qui favorise les petites valeurs tout en conservant une queue plus lourde.", echo=FALSE, message=FALSE, warning=FALSE, dpi = 600}
library(ggplot2)
library(tidyr)
...
results_df <- data.frame(
  Method = rep(c("lm", "brms"), each = 2),
  Parameter = rep(c("Intercept", "Slope"), times = 2),
  Estimate = c(
...
# Assembler côte à côte avec titre et sous-titre globaux
(p_intercept | p_slope) +
...
```{r brms-fit-plot, echo=TRUE, fig.cap="Ajustement du modèle linéaire par brms. La droite bleue est la régression estimée, obtenue en fixant l'ordonnée à l'origine et la pente à leur moyenne a posteriori, entourée de son intervalle de crédibilité à 95 %.", dpi = 600}
...
```{r ppcheck-brms, echo = TRUE, fig.cap = "Posterior predictive checks réalisés avec brms. La courbe noire correspond aux données observées, les courbes bleues aux données simulées selon le modèle. L’axe des abscisses représente les valeurs possibles de la variable réponse simulée ou observée. L’axe des ordonnées indique leur densité estimée.", dpi = 600}
pp_check(lm.brms)
```
